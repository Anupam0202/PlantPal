...
const DetailSection: React.FC<{ title: string; content?: string }> = ({ title, content }) => {
  ...
  return (
    <div className="py-1"> {/* Added padding for better separation */}
      <h4 className="text-sm font-semibold text-emerald-700 dark:text-emerald-300 mb-1 uppercase tracking-wider">{title}</h4> {/* Enhanced title styling */}
      <p className="text-sm text-slate-700 dark:text-slate-200 leading-relaxed whitespace-pre-line">{content}</p> {/* Dark mode text and pre-line for newlines */}
    </div>
  );
};
    ...
    >
      ...
      >
        ...
        <div className="flex justify-between items-start mb-3 sm:mb-4 flex-shrink-0">
          <div>
            <h2 id="plant-detail-title" className="text-xl sm:text-2xl font-bold text-emerald-700 dark:text-emerald-300">{plant.commonName}</h2>
            {plant.scientificName && <p className="text-xs text-slate-500 dark:text-slate-400 italic">({plant.scientificName})</p>}
          </div>
          <button
            onClick={onClose}
            className="text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300 transition-colors p-1 -mr-1 -mt-1 rounded-full focus:outline-none focus:ring-2 focus:ring-slate-400 dark:focus:ring-slate-500"
