...
import { validateApiKeyWithGemini, getPlantRecommendationsFromGemini } from './services/geminiService';
...
const App: React.FC = () => {
    ...
    const initializeApp = async () => {
      ...
      try {
        if (!process.env.API_KEY) {
            throw new Error("API_KEY environment variable is not configured by the administrator.");
        }
        const isValid = await validateApiKeyWithGemini();
        if (isValid) {
          setIsApiKeyValid(true);
        } else {
          setIsApiKeyValid(false);
          setInitializationError("The pre-configured API Key is invalid. Please contact the administrator to ensure it is correct and has the necessary permissions.");
        }
      } catch (err) {
        setIsApiKeyValid(false);
        const message = err instanceof Error ? err.message : "An unknown error occurred during app initialization.";
        setInitializationError(`Initialization failed: ${message}`);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  if (!isApiKeyValid) { 
      return (
          <div className="min-h-screen flex flex-col justify-center items-center bg-white dark:bg-slate-900 p-6 text-center">
              <PlantPalIcon className="w-20 h-20 sm:w-24 sm:h-24 text-emerald-700 dark:text-emerald-500 mb-5" />
              <h1 className="text-2xl sm:text-3xl font-semibold text-green-800 dark:text-green-300">PlantPal Unavailable</h1>
              <p className="text-md text-green-700 dark:text-green-400 mt-2">Initialization failed. Please refresh or contact support.</p>
               <footer className="absolute bottom-8 text-sm text-green-700 dark:text-green-500">
                <p>&copy; {new Date().getFullYear()} PlantPal.</p>
              </footer>
          </div>
      );
